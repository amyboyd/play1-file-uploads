...
abstract public class AbstractUploadedImage extends Model {
    ...
    public void deleteAllFiles() {
        for (Variant variant: getVariants()) {
            ...
            if (USE_S3) {
                S3_CLIENT.deleteObject(AWS_BUCKET_NAME, getPath(variant));
            }
        }
    }
    ...
    private void putFileInCorrectPlace(File file) {
        ...
        if (USE_S3) {
            final PutObjectRequest request = new PutObjectRequest(AWS_BUCKET_NAME, getPath(getVariantFull()), target);
    ...
    final protected void generateVariant(final Variant variant) {
        ...
        if (USE_S3) {
            final PutObjectRequest request = new PutObjectRequest(AWS_BUCKET_NAME, getPath(variant), target);
            request.setCannedAcl(CannedAccessControlList.PublicRead);
            S3_CLIENT.putObject(request);
        }
    }
